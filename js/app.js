...
function renderModuleRecommendations(recommendedModules) {
    ...
    container.innerHTML = `
        <div class="module-rec-grid" onclick="showPage('modules')">
...
async function clearHistory() {
    showConfirm({
        title: 'Hapus Riwayat',
        message: 'Hapus semua riwayat asesmen? Tindakan ini akan menghapus semua data secara permanen.',
        icon: '🗑️',
...
function initStickyProgress() {
    ...
    const observer = new IntersectionObserver(
        ([entry]) => {
            // Tambah class is-sticky saat elemen menempel di atas
            progressEl.classList.toggle('is-sticky', !entry.isIntersecting);
        },
        {
            // Deteksi saat bagian atas elemen menyentuh navbar (top: 64px)
            rootMargin: '-64px 0px 0px 0px',
            threshold: 1.0
        }
    );

    // Buat sentinel element di atas progress bar
...
// Panggil saat halaman assessment dibuka
document.addEventListener('DOMContentLoaded', () => {
    initStickyProgress();
});